...
func (v *VScan) Tagetsacn(addr []string, thread int) {
	...
	aliveHost := make(chan string, thread)
	//go func() {
			...
			if err == nil {
				if result.Service.Name == "http" {
					...
					if result.Service.Extras.Version != "" {
						info = "\n - 版本: " + result.Service.Extras.Version + info
					}
					if result.Service.Extras.VendorProduct != "" {
						info = "\n - 厂商: " + result.Service.Extras.VendorProduct + info
					}
					if result.Service.Extras.Sign != "" {
						info = "\n - 签名: " + result.Service.Extras.Sign + info
					}
					...
					if result.Service.Extras != (Extras{}) {
						if result.Service.Extras.Version != "" {
							info = info + "\n - 版本: " + result.Service.Extras.Version
						}
						if result.Service.Extras.VendorProduct != "" {
							info = "\n - 厂商: " + result.Service.Extras.VendorProduct + info
						}
				...
				host = append(host, result.IP+" 端口:"+strconv.Itoa(result.Port)+info)
				mutex.Unlock()
			}
			<-limiter
		}(targetIP)
